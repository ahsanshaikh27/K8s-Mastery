...
* Use standardized keys (e.g., `app.kubernetes.io/name`, `app.kubernetes.io/version`).
...
1. Deploy the new version with a different label (e.g., `version: v2.0`).
2. Update the Service to point to the new version.
3. Delete the old version after verification.
...
1. Deploy `v2` with fewer replicas and a unique label.
2. Configure the Service to include both `v1` and `v2` Pods.
3. Gradually scale `v2` replicas and reduce `v1` replicas.
...
Standardize labels across resources.
...
Use ArgoCD or Flux for GitOps-driven deployments.

### Rollback Planning

Test rollbacks in staging and use `kubectl rollout undo`.
